<?php
	add_shortcode( 'team-up-member', function( $atts ){
		...
		<div class="team-up team-up-grid">
			<?php
				// Initialize Variables
				$team_member = get_page_by_title( $name, 'OBJECT', 'team-up');
				$meta        = array();

				$content     = apply_filters( 'the_content', $team_member->post_content );
				$limit       = 110;
				...
				if( ! $profile_picture = get_the_post_thumbnail_url( $team_member->ID, 'TeamUp Tall (Crop)' ) ){
